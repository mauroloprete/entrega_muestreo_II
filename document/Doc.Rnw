...
modelo_rf = rand_forest( trees = 200) %>%
  ...
  fit(R ~ estrato + sexo + edad + dpto + id_hogar, data = muestra)

# modelo_rf <- boost_tree(trees = 1000) %>%
#  set_engine('xgboost') %>%
# set_mode('classification') %>% 
...
matriz_confusion <- (data = bind_cols(select(muestra, R),
                          select(pred_rf,.pred_class)),
         truth = R, 
         estimate = .pred_class)
...
muestra <- muestra %>% mutate(w_nr_rf = ifelse(R==1, base_weights/prop_rf,0))
@
...
est_ponderados_nr_rf %>%
    ...
    kbl(
        booktabs = T,
        caption = "Estimaciones poblacionales usando ponderadores por no respuesta utilizando random forest"
    ) %>%
    kable_styling(
        latex_options = c(
            "striped",
...
<<>>=
clases_prop = out = pclass(formula = R~estrato+sexo+edad+id_hogar+dpto,
                           data = muestra, 
                           type = "unwtd", 
                           link="logit", numcl=5) %>%
  as_tibble()

muestra <- bind_cols(muestra, clases_prop)
muestra <- muestra %>% 
  group_by(p.class) %>%
  mutate(ajuste_nr_clases=1/median(propensities)) %>%
  ungroup()
@
...
est_ponderados_nr_clases %>%
    ...
    kbl(
        booktabs = T,
        caption = "Estimaciones poblacionales usando ponderadores por no respuesta utilizando las propensiones del punto anterior"
    ) %>%
    kable_styling(
        latex_options = c(
            "striped",
